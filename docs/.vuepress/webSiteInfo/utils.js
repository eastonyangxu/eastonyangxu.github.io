...
export function dayDiff(startDate, endDate) {
  ...
  let day = parseInt(
    Math.abs(new Date(startDate) - new Date(endDate)) / (1000 * 60 * 60 * 24)
  );
  return day;
}
...
export function timeDiff(startDate, endDate) {
  ...
  if (diffValue == 0) {
    ...
    parseInt(diffValue / (60 * 60 * 24)) <
    getDays(startDate.getMonth, startDate.getFullYear)
  ) {
    return parseInt(diffValue / (60 * 60 * 24)) + " 天";
  } else if (
    parseInt(
      diffValue /
        (60 * 60 * 24 * getDays(startDate.getMonth, startDate.getFullYear))
    ) < 12
  ) {
    return (
      parseInt(
